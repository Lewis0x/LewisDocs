...
- **MicroStation 是基础设施工程领域的 API 设计教科书**：从 1990 年代初的 [MDL](/glossary#mdl-microstation-development-language) 起步<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>，到 V8 时代的 C++ <Term def="V8（2001）后期推出的 Bentley 原生 C++ SDK，分布在 DgnPlatform / DgnGeoCoord 等命名空间下。CONNECT Edition 时代成熟。是 MDL 之后的“现代化 SDK”。">MicroStationAPI</Term>、<Term def="MicroStation V10（2015）的市场名。重大现代化版本：64 位 + Office 风格 Ribbon UI + 成熟的托管 .NET API（Bentley.MicroStation.dll 等）。">CONNECT Edition</Term> 的 .NET 托管层，再到 2024 年正式引入官方 Python API<sup>[官方 13]</sup>，跨越 30+ 年的 API 演进史几乎覆盖了所有主流嵌入式扩展范式。
- **ECObjects/ECSchema 是 Bentley 独有的"工程内容元数据"系统**<sup>[官方 17]</sup>，强类型、可关系建模、XML/JSON 序列化，是 MicroStation 区别于 AutoCAD 等平台的核心竞争力。
- **iTwin Platform 是当今 CAD 厂商中较为激进的云原生重构**<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>：[iModel](/glossary#imodel-imodelhub) = SQLite + [ChangeSet](/glossary#changeset) 时间线，iTwin.js 是开源 TypeScript SDK（**MIT License**<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a>[新闻 8]</sup>，社区资料中常见误写为 Apache 2.0，需以官方为准）。截至 2025 年 iTwin.js 已迭代至 5.x 系列，包名从 `@bentley/*` 全面迁移到 `@itwin/*`<sup><a href="https://www.itwinjs.org/changehistory/3.0.0/" target="_blank" rel="noreferrer">[官方 9]</a></sup>。
- **MicroStation 与 iTwin 是"桌面 + 云"的双轨而非替代关系**<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>：MicroStation 仍是设计原生、DGN 仍是文件标准；iTwin 是基于多源数据派生的"数字孪生联邦"。
- **DGN 文件格式自 V8（2000 年创建、随 2001 年 MicroStation V8 发布<sup><a href="https://en.wikipedia.org/wiki/DGN" target="_blank" rel="noreferrer">[百科 4]</a></sup>）改用 <Term def="Microsoft 1990s 设计的“文件内嵌文件系统”格式：单个磁盘文件内嵌多个流（streams）和子目录（storages）。Office .doc / .xls 早期版本、MSI 安装包、MicroStation V8 DGN 都用它。">OLE Compound File</Term>**；ODA 在 **2003 年**宣布支持 V8 DGN<sup><a href="https://en.wikipedia.org/wiki/DGN" target="_blank" rel="noreferrer">[百科 4]</a></sup>，**V8 DGN File Format Reference 公开于 2005 年**<sup><a href="https://grokipedia.com/page/DGN（关于" target="_blank" rel="noreferrer">[百科 5]</a></sup>。⚠️ 社区资料中常见的"2005 年公开规范换来 ODA 支持"说法时间序列不准确——ODA 支持发生在规范公开之前。
- **垂直产品矩阵高度成熟**：OpenRoads / OpenBuildings / OpenRail / OpenBridge / OpenPlant 等 OpenX 系列建立在 MicroStation + ECSchema 之上<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>。
...
7. **ECSchema 是工程级元数据系统**：支持 ECClass / ECRelationshipClass / ECStructProperty / ECNavigationProperty / CustomAttribute<sup>[官方 17]<a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>，是 BIS（Base Infrastructure Schemas）和 iModel 的概念骨架<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>。
8. **iTwin.js 三段架构（Frontend/Backend/Common）**：通过 RpcInterface 统一通信抽象<sup><a href="https://www.itwinjs.org/learning/" target="_blank" rel="noreferrer">[官方 19]</a></sup>。
9. **iModel = SQLite + ChangeSet**：每个 iModel 在 iModelHub 注册一个 GUID，本地工作副本叫 [Briefcase](/glossary#briefcase)（一个 SQLite 文件），修改后推送 Changeset 到 iModelHub<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a><a href="https://www.itwinjs.org/learning/" target="_blank" rel="noreferrer">[官方 19]</a></sup>。
10. **iTwin.js 的开源策略与 Microsoft Azure 战略合作**：⚠️ iTwin.js 自 2018 年 10 月 17 日开源——核心 `iTwin/itwinjs-core` 仓库为 **MIT License**（多处社区资料误传为 Apache 2.0）<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a>[新闻 7][新闻 8]</sup>；周边的 `imodeljs-native`（C++ 二进制组件）使用 **Apache 2.0 + Bentley 商业 right-to-run 双许可**——后者控制非 Snapshot iModel 的访问。整体依赖 SQLite、Node.js、NPM、WebGL、Electron、Docker、Kubernetes 等开放技术<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>。
...
DGN 文件格式起源于 1979 年 Intergraph Graphics Design System (IGDS)<sup><a href="https://en.wikipedia.org/wiki/DGN" target="_blank" rel="noreferrer">[百科 4]</a></sup>。Bentley 兄弟（Keith 和 Barry）作为 Intergraph 的 OEM 合作方<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>，在 1984 年发布 PseudoStation——一个让 PC 终端可访问 IGDS 数据的程序<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>。1985 年 MicroStation 1.0 发布，作为 IGDS 在 PC 上的克隆工具，**最初只能读 DGN，不能写**；1987 年 MicroStation 2.0 才支持读写<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>。

### 1.2 MDL 诞生：MicroStation V4（1990 年代初）

⚠️ **关于 MDL 引入时间的多源分歧**：

- Wikipedia "MicroStation"：The 1992 release of version 4 introduced the ability to write applications using MDL<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>
- Sumbera 的 MDL 历史教程：MDL 在 MicroStation v.4 in year 1991 引入<sup><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>
- LA Solutions：MDL has been available since 1993<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>
- 书籍《Programming With MDL: The MicroStation Development Language (Microstation Version 4.X)》早期出版<sup>[书籍 3]</sup>

这种分歧可能源于：MicroStation 4.0 在 1990 年底发布<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>，4.x 系列陆续到 1993 年；MDL 可能在 4.x 系列中逐步成熟，不同来源以不同里程碑事件作为"引入年份"。本报告采用"V4 系列（1990 年代初）"的稳妥表述。

MDL 是一种 C 语法子集 + 内置库函数 + p-code 字节码的设计<sup><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>：

- 开发者编写 `.mc` 源文件（C 风格语法），用 Bentley 的 `mdlcc` 编译器编译为 `.ma` p-code 字节码
- 用 `bmake` 工具管理构建（自研 make 替代品）
- MicroStation 内置 p-code 解释器加载 `.ma` 并执行
- 一套源码 + 一份 `.ma` 即可在 DOS、Win3.x、SunOS、HP-UX、IRIX、Mac 等所有 MicroStation 支持的平台运行<sup><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>
...
> **[推论]** MDL 替代了之前的 MicroCSL（pre-MDL 链接库）和 UCM（User Commands）<sup><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>，其设计动机包含：(1) 跨平台代码兼容性，(2) 提供专业级开发框架以替代简单的 macro 工具。
...
MicroStation/J（即 MicroStation V7）于 SE（1997）发布约一年后推出<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>。"J" 代表 Java——这一版本引入了 Java 增强的 MDL 变种 **JMDL**<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>。

JMDL 是介于 C++ 和 Java 之间的语言（"Java with pointers"），与 ECM（Engineering Component Model，早期的"工程对象组件模型"实验）配套推广<sup><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>。JMDL 在 MicroStation Geographics 的 SDODGN API 中使用过<sup><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>。

但 JMDL 很快被放弃。Bentley 工程师的一段公开表述：

> "we (Bentley) are not starting any new JMDL projects. However, we do have a lot of JMDL code inside MicroStation and we do use a lot of Java in our server applications."<sup><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>
...
V8 是 MicroStation 历史上最重要的版本之一，Wikipedia 称之为 "the most groundbreaking release in Bentley's history"<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>。

**DGN V8 文件格式的关键变化**<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a><a href="https://en.wikipedia.org/wiki/DGN" target="_blank" rel="noreferrer">[百科 4]</a>[第三方 7]</sup>：
...
**MDL 演变**：从 V8 起 MDL 不再是独立语言，转变为"MicroStation Development Library"——可直接编译为 Windows DLL，可在 Visual Studio 中调试，可与任意语言互操作<sup><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>。

**V8 DGN 格式的开放路径**：

- **2000 年**：Bentley 创建了 V8 DGN 格式（Wikipedia 表述为 "In 2000, Bentley Systems created an updated version of DGN"）<sup><a href="https://en.wikipedia.org/wiki/DGN" target="_blank" rel="noreferrer">[百科 4]</a></sup>
- **2001 年**：随 MicroStation V8 发布<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>
- **2003 年**：OpenDWG Alliance（即后来的 Open Design Alliance, ODA）与 Bentley 合作宣布支持 V8 DGN<sup><a href="https://en.wikipedia.org/wiki/DGN" target="_blank" rel="noreferrer">[百科 4]</a></sup>
- **2005 年**：V8 DGN File Format Reference 公开<sup><a href="https://grokipedia.com/page/DGN（关于" target="_blank" rel="noreferrer">[百科 5]</a></sup>，提供详细规范给开发者
- **2008 年**：Autodesk 与 Bentley 互让软件库（Autodesk RealDWG 与 Bentley 的 DGN 库）<sup><a href="https://en.wikipedia.org/wiki/DGN" target="_blank" rel="noreferrer">[百科 4]</a></sup>

⚠️ **重要事实澄清**：社区资料中常见的"2005 年公开规范是 ODA 支持的换取条件"说法不准确——根据 Wikipedia 时间线，ODA 实际在 **2003 年**就宣布支持 V8 DGN<sup><a href="https://en.wikipedia.org/wiki/DGN" target="_blank" rel="noreferrer">[百科 4]</a></sup>，2005 年 V8 DGN File Format Reference 公开是规范文档发布事件，发生在 ODA 支持之后。
...
CONNECT Edition（内部版本号 V10）的关键变化<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>：

- 64-bit 架构
- Microsoft Office 风格 Ribbon UI（取代传统 toolbar）
- **MicroStationAPI（C++ 原生）成熟**：`DgnPlatform` 命名空间，`DgnFile` / `DgnModel` / `DgnAttachment` / `DgnModelRef` 完整层级<sup><a href="https://www.la-solutions.org/CONNECT/MicroStationAPI/MicroStationAPI-CellHandlers.htm" target="_blank" rel="noreferrer">[第三方 15]</a></sup>
- **DgnPlatformNET（.NET 托管层）**：`Bentley.MicroStation.dll` / `Bentley.DgnPlatformNET.dll` / `Bentley.GeometryNET.dll`<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>
...
- **开源示例仓库**：[`BentleySystems/MicroStationPython`](https://github.com/BentleySystems/MicroStationPython) 公开实现、示例、构建脚本<sup><a href="https://github.com/BentleySystems/MicroStationPython" target="_blank" rel="noreferrer">[官方 12]</a></sup>
...
| 层 | 引入时间 | 形态 | 来源 |
|---|---|---|---|
| MDL | 1990 年代初<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup> | C 函数库 + .ma p-code（legacy）+ Windows DLL | 早期跨平台需求 |
| MicroStationAPI (C++) | V8 时代（2001+）<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup> | C++ 类库，`DgnPlatform` 命名空间 | 原生性能与深度扩展 |
| DgnPlatformNET (.NET) | CONNECT Edition (2015+)<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup> | C#/VB.NET 托管程序集 | 现代企业开发 |
| Python API | MicroStation 2024<sup>[官方 13]</sup> | Python 3.12 绑定 | 设计师/自动化脚本 |

此外还有 VBA（V8+ 引入<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>）作为 legacy 脚本路径。
...
`DgnFile` 持有一组 `DgnModel`，每个 model 是独立的设计空间<sup><a href="https://www.la-solutions.org/CONNECT/MicroStationAPI/MicroStationAPI-CellHandlers.htm" target="_blank" rel="noreferrer">[第三方 15]</a></sup>。Models 是 V8 引入的概念<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>——V7 时代一个 DGN 就只有一个隐式 model。
...
每种元素类型对应一个 ElementHandler 单例（无状态），方法接受 ElementHandle 参数<sup><a href="https://www.la-solutions.org/CONNECT/MicroStationAPI/MicroStationAPI-CellHandlers.htm" target="_blank" rel="noreferrer">[第三方 15]</a></sup>：
...
MicroStation 的几何系统**不直接使用 Parasolid/ACIS 作为 B-Rep 内核**（与 SolidWorks/NX 不同），而是有自己的几何抽象层<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>：
...
MicroStation 在元素上附加自定义数据有四个层次，从轻到重<sup>[官方 17]<a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>：

### 5.1 Linkage（V7 时代）

附加在元素后的二进制小段，固定结构<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>。线型、颜色 override 等系统级 linkage 也是这个机制。开发者可定义自己的 linkage ID，但数据是字节流，需自己解析。
...
> **[推论]** 根据公开 ECObjects 文档<sup>[官方 17]<a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>，Item Types 实际上是 ECSchema 的简化版，内部存储格式都是 EC 系统。本报告未在官方文档中找到对此实现细节的直接陈述，属基于 Bentley 元数据栈架构的合理推断。
...
ECObjects 是 Bentley 自研的强类型工程数据元模型<sup>[官方 17]<a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>，可类比为 OWL/RDF 但更工程化。它是 ProjectWise、AssetWise、iTwin 的共同基础<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>。

### 6.1 概念体系

根据 iTwin.js 官方 ECSchema 文档<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>：
...
ECSchema XML 命名空间为 `http://www.bentley.com/schemas/Bentley.ECXML.3.2`<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>。一个简化示例：
...
- **ProjectWise 中**：作为元数据骨架被 ProjectWise Explorer 和 AssetWise 共享<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>
- **iTwin 中**：是 iModel 的核心结构（详见下章 BIS）<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>

### 6.4 BIS（Base Infrastructure Schemas）

BIS 是 iModel 的概念骨架，由 ECSchema 表达<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>：
...
iTwin.js 文档明确规则：除 BisCore 外所有 ECClass 需要继承自 BisCore 的某个 ECClass<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>。
...
MicroStation 是平台底座，OpenRoads / OpenSite Designer / OpenRail / OpenBridge / OpenBuildings / OpenPlant / OpenTower / OpenWindPower / Substation / PowerRebar 等 OpenX 系列垂直应用建立在其之上<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>。

### 7.1 关键设计决定

这些产品**安装时附带完整的 MicroStation 引擎**<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>。垂直产品 = MicroStation 引擎 + 行业 ECSchema + UI 定制 + 工作流。
...
例如 OpenRoads 的 Civil schema 决定了一个 .dgn 文件是 ORD 项目还是裸 DGN<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>。

> **[推论]** 根据社区讨论<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>，用较新版 ORD 打开会**升级 civil schema**，导致老版本只能只读打开（"Newer Civil Data Found"提示）——这是典型的 schema 单调演化问题，与软件版本管理中的"破坏性 schema 升级"困境同构。本报告未在 Bentley 官方文档中找到对此机制的明确文字描述，属社区观察。

### 7.3 GenerativeComponents（GC）

Bentley 的参数化设计工具<sup><a href="https://en.wikipedia.org/wiki/MicroStation" target="_blank" rel="noreferrer">[百科 1]</a></sup>，基于 C# API（`Bentley.GenerativeComponents.ElementBasedNodes`）<sup><a href="https://www.itwinjs.org/learning/" target="_blank" rel="noreferrer">[官方 19]</a></sup>，类似 Grasshopper 但深度集成 MicroStation。
...
⚠️ **重要事实澄清**：社区资料中常见将 iTwin.js 误述为 **Apache 2.0** 许可证——**这是错误的**。多家媒体（包括 Bentley 官方新闻稿引用方）确认开源许可证为 **MIT License**<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a>[新闻 7][新闻 8]</sup>。

iTwin.js 的前身名为 **iModel.js**，**2018 年 10 月 17 日** Bentley 在 Year in Infrastructure 2018 大会（伦敦）上宣布初始版本开源<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a>[新闻 7]</sup>：

> "Bentley Systems, Incorporated, the leading global provider of comprehensive software solutions for advancing the design, construction, and operations of infrastructure, today announced the initial release of its iModel.js library, an open-source initiative to improve the accessibility, for both visualization and analytical visibility, of infrastructure digital twins."<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>
> 
> "The source code is hosted on GitHub and is distributed under the MIT license."<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>

iModel.js 依赖的开放技术栈包括<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>：SQLite、Node.js、NPM、WebGL、Electron、Docker、Kubernetes、HTML5/CSS。

### 8.2 包名演进

- **v1.0–v2.x**（2019–2021）：包名为 `@bentley/imodeljs-*`<sup><a href="https://www.itwinjs.org/changehistory/3.0.0/" target="_blank" rel="noreferrer">[官方 9]</a></sup>
- **v3.0**（2022）：迁移至 `@itwin/core-*` 命名空间，移除 SAML 认证支持<sup><a href="https://www.itwinjs.org/changehistory/3.0.0/" target="_blank" rel="noreferrer">[官方 9]</a></sup>
- **v5.x**（2025）：支持 Node.js 24，TypeScript 升级至 ES2023/ES2022<sup><a href="https://www.itwinjs.org/changehistory/" target="_blank" rel="noreferrer">[官方 10]</a></sup>

### 8.3 三段架构

iTwin.js 设计为可同时部署为 Web、移动、桌面三种形态<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>：
...
**iModel = SQLite + ChangeSet + ECSchema**<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a><a href="https://www.itwinjs.org/learning/" target="_blank" rel="noreferrer">[官方 19]</a></sup>：

- 每个 iModel 在 iModelHub 注册一个 GUID
- 本地工作副本叫 **Briefcase**——一个 SQLite 文件
- 修改后推送 **Changeset** 到 iModelHub
- iModelHub 要求 changeset 基于最新版本（线性时间线）<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>
- 每个 Changeset 有 `Id`（内容 hash 字符串）和 `Index`（线性整数）双重标识<sup><a href="https://www.itwinjs.org/changehistory/3.0.0/" target="_blank" rel="noreferrer">[官方 9]</a></sup>
- 命名版本（Named Version）在时间线上打 tag<sup><a href="https://www.itwinjs.org/changehistory/3.0.0/" target="_blank" rel="noreferrer">[官方 9]</a></sup>

iTwin.js 4.x 起支持 changeset rebase（合并 changeset）<sup><a href="https://www.itwinjs.org/changehistory/" target="_blank" rel="noreferrer">[官方 10]</a></sup>。
...
> **[推论]** iTwin 的"federated"特征（不要求所有数据都搬到 iModel 内，可允许 reality data/IoT/markup 保留在外部存储）是它独有的设计选择<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>。这反映了基础设施工程的现实需求：数据源极度异构（DGN、DWG、RVT、IFC、点云、IoT、GIS 等），强行迁移到单一权威数据库会失败。

### 8.6 iTwin Connector：增量同步

iTwin Connector（前称 iModel Bridge）把 DGN/DWG/RVT/IFC/PDS/Smart 3D/AVEVA PDMS/E3D 等转为 iModel<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>。
...
iTwin 在 iModel 上提供 **ECSQL**，基于 ECSchema 的 SQL 方言<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>：
...
**事实依据**：
- iTwin.js 完全开源（MIT License）<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>
...
- **开源示例仓库**：[BentleySystems/MicroStationPython](https://github.com/BentleySystems/MicroStationPython)<sup><a href="https://github.com/BentleySystems/MicroStationPython" target="_blank" rel="noreferrer">[官方 12]</a></sup>
...
> **[推论]** Python API 是 MicroStationAPI（C++）的薄包装，通过 C 扩展（类似 Boost.Python / pybind11）暴露<sup><a href="https://github.com/BentleySystems/MicroStationPython" target="_blank" rel="noreferrer">[官方 12]</a></sup>。本报告未在官方文档中找到对绑定层具体实现机制的详细陈述，"类似 Boost.Python / pybind11"属基于公开仓库代码的推断。
...
MicroStation 是较早把"工程元数据"作为一等公民设计的 CAD 平台之一。从 1990s 末的 ECM 实验<sup><a href="http://www.sumbera.com/ustation/tutorial/mdlevolution.htm" target="_blank" rel="noreferrer">[第三方 2]</a></sup>到 ECObjects 成熟，再到 BIS schemas 的体系化<sup><a href="https://www.itwinjs.org/bis/ec/ec-schema/" target="_blank" rel="noreferrer">[官方 18]</a></sup>，Bentley 把"工程数据 = 几何 + 强类型语义"作为核心信念。这与 AutoCAD 的"几何为主、属性为辅"哲学形成对比。

### 10.2 "桌面 + 云"双轨而非"云替代桌面"

iTwin 没有取代 MicroStation 的野心——MicroStation 仍是设计原生工具，DGN 仍是文件标准；iTwin 是基于多源数据派生的"联邦数字孪生层"<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>。这与 Autodesk Fusion 360（云原生但试图取代部分桌面功能）路线不同。

### 10.3 "联邦数据"而非"单一权威"

iModel 不要求所有数据都搬入<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>——reality capture、IoT、markup 等可保留在外部，通过引用聚合。这是基础设施工程的现实需求（数据源极度异构，强行迁移会失败）。
...
iTwin.js 完全开源（MIT License）<sup><a href="https://www.businesswire.com/news/home/20181017005008/en/" target="_blank" rel="noreferrer">[新闻 6]</a></sup>+ 与 Microsoft Azure 深度合作 + 不自建 IaaS。这是基础设施 ISV 的务实选择——Bentley 的核心竞争力在领域知识与 schema 体系，不在云基础设施。
...
- **DGN 在中国的开放度**：虽然 V8 DGN File Format Reference 2005 公开<sup><a href="https://grokipedia.com/page/DGN（关于" target="_blank" rel="noreferrer">[百科 5]</a></sup>，但 ODA Drawings SDK 对 DGN 支持深度仍不及 DWG，国产 CAD 厂商兼容 DGN 的成本较高（属社区观察）。
...
- **MDL 是否还能用于新开发**：技术上 CONNECT Edition 仍兼容 MDL .ma 文件，但 Bentley 官方推荐 C++/MicroStationAPI 或 .NET DgnPlatformNET<sup><a href="https://www.la-solutions.org/CONNECT/MicroStation-CONNECT.htm" target="_blank" rel="noreferrer">[第三方 14]</a></sup>。新项目用 MDL 是负债。
- **MicroStation Python API 仍在快速演化**：2024 首发，2025 年仍在补充 API 覆盖范围（GitHub `BentleySystems/MicroStationPython` 仓库 README 明确"working on exposing every feature"<sup>[第三方 11]<a href="https://github.com/BentleySystems/MicroStationPython" target="_blank" rel="noreferrer">[官方 12]</a></sup>）。
