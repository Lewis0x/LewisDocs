...
> **[评论]** 声明式范式的优势是"参数化关联性"自然——用户调整参数时引擎重算，下游自动传播。命令式范式需要额外维护依赖图。**适用边界**：此趋势在参数化 CAD 中明显，对命令式 API 主体的平台（AutoCAD、SketchUp）适用性有限——这些平台的命令式 API 仍是主流且未必会被取代。新平台选择 feature API 范式时建议根据客户基础匹配。
