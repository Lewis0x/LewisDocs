...
- **SketchUp 是"嵌入式脚本极简哲学"的代表**：Ruby 解释器嵌入到 SketchUp 进程，Ruby Console 提供即时 <Term def="Read-Eval-Print Loop。交互式命令行：输入一行代码立即求值并打印结果，无需编译重启。Lisp/Python/Ruby 等动态语言的标志性体验，便于探索式编程">REPL</Term> 体验,扩展打包格式简单(<Term def="SketchUp 扩展打包格式。本质是 zip 压缩包,内含一个根 .rb 文件 + 同名子目录,可选数字签名与 .rbe 加密,通过 Extension Warehouse 分发">`.rbz`</Term> 即 zip + manifest)<sup>[官方 1]</sup>。这种"低门槛 + 高扩展性"的组合让设计师友好的脚本生态成为可能。
- **几何模型刻意停留在 <Term def="以“平面多边形面”为最小建模单位的几何系统。SketchUp 不存储真正的 B-Rep 拓扑，所有 Face 是平面 + Edge 是直线段，曲面用密集小三角面片近似。代价是高级几何能力(NURBS/参数化)需扩展弥补，收益是实现简单 + 性能极佳">face-based</Term> + 简单拓扑**：核心类是 `Sketchup::Edge` / `Sketchup::Face` / `Sketchup::Loop` / `Sketchup::Group` / `Sketchup::ComponentInstance`<sup>[官方 1]</sup>。**没有 NURBS、没有 B-Rep、所有 Face 是平面多边形**——这极大降低了实现复杂度，也是 SketchUp 性能传奇的来源。Solid Tools（Union/Subtract/Trim/Intersect）只对"无泄漏的封闭体积"有效。
- **17+ 类 [Observer](/glossary#observer-观察者模式) 提供高粒度事件**：`Sketchup::AppObserver / ModelObserver / EntitiesObserver / EntityObserver / SelectionObserver / ToolsObserver / ViewObserver` 等<sup>[官方 1][第三方 2]</sup>，比 AutoCAD Reactor / Inventor TransactionEvents 的颗粒度都更细。
- **UI::HtmlDialog 是 Web 集成的现代化路径**：SketchUp 2017 引入<sup>[官方 3][官方 6]</sup>，嵌入 <Term def="Chromium Embedded Framework。把 Chromium 浏览器内核以 C++ 库形式嵌入桌面应用的开源框架，让原生程序能托管 Web UI 并与 JS 双向通信。SketchUp/Steam/Spotify 等都用它做内嵌 Web 面板">Chromium Embedded Framework (CEF)</Term>，跨 Windows/Mac 行为一致。CEF 版本演进：SU2017 → 52、SU2018 → 56、SU2019 → 64、SU2021.1 → 88、SU2024 → 112<sup>[官方 3]</sup>。⚠️ 老版本 `UI::WebDialog`（依赖 IE/WebKit）已弃用<sup>[官方 4]</sup>。
...
- **SketchUp 2025–2026 是新一代演进期**：SU 2025 引入 <Term def="逻辑像素。在高 DPI（Retina/4K）显示器上，把“1 像素”抽象为与物理分辨率无关的逻辑单位，由 OS 按缩放系数映射到实际像素。让 UI 与绘图在不同 DPI 屏上保持一致大小，避免老代码在高分屏上字体/图标缩成针尖">logical pixels</Term>（DPI scaling）和 Snaps Ruby API<sup>[官方 7]</sup>；SU 2026 引入 **<Term def="SketchUp 2026 新增的扩展渲染 API。允许扩展在 SketchUp 原生工具运行期间持续叠加绘制（如标注、辅助线、可视化层），打破“扩展只能在自己 Tool 激活时画图”的传统约束">Overlays API</Term>**（扩展可在原生工具运行时持续渲染）和 **AI Render / AI Assistant**（2026.1）<sup>[官方 8][新闻 9]</sup>；SU 2026 要求 scene edits 用 `start_operation/commit_operation` 包装，否则会被 Extension Warehouse 拒绝<sup>[官方 8]</sup>。
- **Trimble 时代的战略走向**：SketchUp 自 2012 年从 Google 转让给 Trimble<sup>[百科 10]</sup>，逐步整合 Trimble Connect（云协作）、3D Warehouse（资产）、Modus（设计系统）。但 **SketchUp for Web 至今不支持 Ruby 扩展**<sup>[第三方 11]</sup>——这是 Trimble "保守上云" 的鲜明立场。

---

## Key Findings

1. **嵌入式 Ruby 是 SketchUp API 的核心**：标准 <Term def="Matz's Ruby Interpreter。Ruby 之父 Matz 用 C 实现的官方参考解释器，长期是 ruby-lang.org 默认实现。区别于 mruby（嵌入式精简版）/JRuby（JVM）/TruffleRuby（GraalVM）。SketchUp 嵌入完整 MRI 以保留 Gem 生态">MRI Ruby</Term>（不是 mruby）嵌入在 SketchUp 进程中，Ruby Console 面板提供即时 REPL<sup>[官方 1]</sup>。Ruby 版本演进：SU2014 → 2.0 / SU2017 → 2.2.4 / SU2019 → 2.5.5 / SU2021 → 2.7.x / SU2024 → 3.2.x<sup>[官方 12][第三方 13]</sup>。
2. **核心对象层级以 `Sketchup` 模块为根**：`Sketchup.active_model` 返回 `Sketchup::Model`，从此处分叉到 `entities` / `layers`（SU2020+ 改名 tags 但 API 保留 layers）/ `materials` / `definitions` / `selection` / `styles` / `shadow_info`<sup>[官方 1]</sup>。
3. **Entity 类型层次清晰**：`Sketchup::Entity` → `Sketchup::Drawingelement` → `{Edge, Face, Group, ComponentInstance, Image, SectionPlane, Text, Dimension, ...}`<sup>[官方 1]</sup>。
4. **Tool 类用 <Term def="鸭子类型。“如果它走起来像鸭子、叫起来像鸭子，那它就是鸭子”——动态语言中不要求显式继承或接口声明，只要对象实现了所需方法即可被使用。SketchUp 自定义 Tool 不需继承基类，只要响应 activate/draw/onMouseMove 等方法即被框架接受">duck-typing</Term> 实现**：自定义工具不需继承基类，只需实现回调方法（`activate / deactivate / draw / onMouseMove / onLButtonDown / onUserText` 等）<sup>[官方 14]</sup>。`Sketchup::InputPoint` 处理推理捕捉，`view.draw / view.draw2d` 在 OpenGL 上下文中绘制临时图形。
...
6. **`.rbz` 扩展打包格式**：实质是 zip 包 + 单个根 `.rb` 文件 + 同名子目录<sup>[官方 15]</sup>。Extension Warehouse 上传时执行**数字签名 + 可选加密**（`.rb` → `.rbe`，需 `Sketchup.require` 加载）<sup>[官方 15]</sup>。
...
8. **SU 2025 引入 Snaps Ruby API**<sup>[官方 7]</sup>，SU 2025.0 全面切换到 logical pixels（应对多 DPI 监视器）。
9. **SU 2026 引入 Overlays API**：扩展可在原生工具运行时持续渲染<sup>[官方 8]</sup>——这是对"扩展只能在自己 Tool 激活时画图"传统约束的重大突破。
10. **SU 2026.1 引入 AI Render 与 AI Assistant**<sup>[官方 17]</sup>：标志 SketchUp 进入生成式 AI 整合阶段。
...
SketchUp 由 @Last Software 创立，初衷是"3D for everyone"——面向设计师而非工程师的草图工具<sup>[百科 10]</sup>。

- 2000 年：SketchUp 1.0 发布<sup>[百科 10]</sup>
- 早期：v4.0 引入 Ruby API + 首批 Observer + Tool 类<sup>[官方 1]</sup>
- 2003 年：Push/Pull 工具获得美国专利 US 6,628,279<sup>[百科 10]</sup>
...
2006 年 Google 收购 @Last Software<sup>[百科 10]</sup>。Google 时代的关键变化：

- 2006 年：SketchUp Free 版发布（免费版本，吸引大量设计师与教育用户）
- SU6 / 7 / 8 系列：Ruby API 持续演进，但 Ruby 解释器版本停留在 1.8.6 较久<sup>[官方 12]</sup>
...
2012 年 Google 将 SketchUp 转让给 Trimble<sup>[百科 10]</sup>。Trimble 时代的主要 API 变化：

| 年份 | 版本 | 关键 API/Runtime 变化 |
|---|---|---|
| 2013 | SU 2013 | 启用年版本号命名<sup>[官方 12]</sup> |
| 2014 | SU 2014 | Ruby 升级到 2.0<sup>[官方 12]</sup> |
| 2017 | SU 2017 | Ruby 2.2.4；**UI::HtmlDialog 引入**（嵌入 CEF 52）<sup>[官方 3][官方 6]</sup> |
| 2018 | SU 2018 | CEF 升级到 56<sup>[官方 3]</sup> |
| 2019 | SU 2019 | CEF 64；Ruby 2.5.5<sup>[官方 3][官方 12]</sup> |
| 2019.2 | SU 2019.2 | C SDK 可在 Ruby C 扩展中只读访问活动模型<sup>[官方 5]</sup> |
| 2021 | SU 2021 | Ruby 2.7.x；Live Components 引入<sup>[官方 12]</sup> |
| 2021.1 | SU 2021.1 | CEF 88；Ruby 2.7.2<sup>[官方 12]</sup> |
| 2023 | SU 2023 | persistent_id 引入；Snap entity 引入<sup>[官方 7]</sup> |
| 2024 | SU 2024 | Ruby 3.2.x；CEF 112；新图形引擎<sup>[官方 3][官方 12]</sup> |
| 2025 | SU 2025 | **Snaps Ruby API**；logical pixels（DPI scaling）；OpenSSL 3.4.1<sup>[官方 7]</sup> |
| 2026 | SU 2026 | **Overlays API**；非可逆变换严格校验；scene edits 需要 wrap operation<sup>[官方 8]</sup> |
| 2026.1 | SU 2026.1 | **AI Render**；**AI Assistant**（含 Help Assistant + Generate Object）<sup>[官方 17]</sup> |
...
SketchUp 的 API 分为两个主要轨道<sup>[官方 1][官方 5]</sup>：
...
`Sketchup` 模块作为根<sup>[官方 1]</sup>，`Sketchup.active_model` 返回 `Sketchup::Model` 实例：
...
参考：官方 `Sketchup::Entity` 类文档<sup>[官方 1]</sup>。

### 3.3 Component / Group / Instance：核心 instancing 机制

`ComponentDefinition` 持有 `entities`，`ComponentInstance` 通过 `Geom::Transformation` 实例化<sup>[官方 1]</sup>。Group 是退化的匿名 ComponentDefinition（`group?` 返回 true）。
...
SketchUp 的 Tool 类通过 duck-typing 实现（不需继承基类）<sup>[官方 14]</sup>。需实现的回调包括：
...
> **[推论]** SketchUp 2026 起要求 scene 修改 wrap 在 `start_operation/commit_operation` 之间，否则扩展将被 Extension Warehouse 拒绝<sup>[官方 8]</sup>。这反映了 SketchUp 对 Undo 栈卫生的逐步严格化——可能是社区长期反馈"扩展污染 Undo 栈"问题的回应。
...
⚠️ **澄清**：SketchUp 的几何模型常被简称为 "face-based"，更精确的描述是<sup>[官方 1]</sup>：
...
SketchUp 的 Solid Tools（Union/Subtract/Trim/Intersect/Outer Shell/Split）只对**无泄漏的封闭体积**有效<sup>[官方 1]</sup>。调用 `ComponentInstance#manifold?` 检查、调用 `Sketchup::Group#split` 之类方法时若拓扑无效直接返回 `nil`。
...
SketchUp 2023 引入了批量几何 API `Sketchup::EntitiesBuilder`<sup>[官方 12]</sup>，用于高速注入大量边/面：
...
根据社区资料整理<sup>[第三方 2][官方 1]</sup>，SketchUp 提供约 17 类 Observer：
...
注：实际数量在不同 SketchUp 版本中略有变化，社区维护的 thomthom.net 列表是较权威参考<sup>[第三方 2]</sup>。
...
> **[评论]** 这是 SketchUp Observer 系统的著名陷阱<sup>[第三方 2]</sup>，新手扩展开发者经常遇到。设计上的根本问题是 SketchUp 的 model lifecycle（新建/关闭）与 observer 绑定 lifecycle 没有自动统一——需要开发者显式管理。这种"显式生命周期"在嵌入式脚本设计中是合理选择（避免隐式 magic），但学习成本不低。
...
SketchUp 2017 引入了 `UI::HtmlDialog`<sup>[官方 3][官方 6]</sup>，替代已废弃的 `UI::WebDialog`<sup>[官方 4]</sup>。

⚠️ **关键差异**：

- `UI::WebDialog`（老）：依赖系统浏览器引擎（Windows IE / Mac WebKit），跨平台行为不一致<sup>[官方 4]</sup>
...
根据官方 `UI::HtmlDialog` 文档<sup>[官方 3]</sup>：
...
> **[评论]** 这种"action_callback + execute_script"双向通信范式与 Electron 的 ipcRenderer/ipcMain 模式高度相似——都是把"Web 前端 + 桌面后端"的边界明确化。优点是 Ruby 端保持业务逻辑、JS 端专注 UI 渲染；缺点是异步通信增加了状态同步的复杂度（API issue tracker 中长期有 drag-and-drop 等场景的 bug 报告<sup>[第三方 16]</sup>）。

### 6.4 Trimble Modus：UI 风格统一

Trimble 提供 **Modus** 设计系统（CSS/JS 库）<sup>[官方 3]</sup>，让扩展 UI 与 SketchUp 主界面观感一致。这是 Trimble 时代的"设计系统"投资——类似 Microsoft Fluent UI 或 Google Material Design 在自家产品中的角色。
...
`.rbz` 实质是 zip 包<sup>[官方 15]</sup>，结构：
...
Extension Warehouse 上传时执行<sup>[官方 15]</sup>：
- **数字签名**：验证扩展完整性
...
SketchUp 2026 起，Extension Warehouse 对扩展提出新的严格要求<sup>[官方 8]</sup>：
...
根据官方 release notes<sup>[官方 7]</sup>：
...
⭐ **Overlays API 是 SU 2026 最重要的扩展能力突破**<sup>[官方 8]</sup>。在此之前，扩展只能在自己的 Tool 激活时绘制信息到 modeling 窗口。Overlays 允许扩展**在其他工具运行时持续渲染**——即"在用户用 Push/Pull 工具时，我的扩展仍在画分析叠加层"。
...
- **非可逆变换严格校验**<sup>[官方 8]</sup>：`Geom::Transformation#inverse` 等方法在零 scale 等情况下抛 `ArgumentError`（之前是 silent failure）
- **Scene 修改强制 wrap operation**：参见 7.3 节
- **统一 IFC exporter**：合并 IFC 2x3 和 IFC 4 为单一 exporter，提供版本与层级选项

### 9.4 SU 2026.1 引入 AI

SU 2026.1 引入 SketchUp AI<sup>[官方 17]</sup>：
...
- **HtmlDialog 跨平台 bug 长期未解**：API issue tracker 中 drag-and-drop、focus 等问题报告多年<sup>[第三方 16]</sup>，开发者社区对 HtmlDialog 质量评价分化。
...
- **本土在线设计软件的分流**：酷家乐、三维家等本土厂商在快速出图、电商导购场景中分流较多用户，但 SketchUp 在专业建筑院校和概念设计阶段仍占较大使用份额。SketchUp 2020 年从永久授权转型订阅制<sup>[百科 10]</sup>与中国设计师习惯的永久授权之间形成鸿沟，是本土在线设计软件抢占市场的窗口。
...
- **CEF 版本演进数据**已根据官方 `UI::HtmlDialog` 文档<sup>[官方 3]</sup>核实更新；早期资料可能引用过旧的 CEF 版本。
- **Ruby 版本演进数据**来自 SketchUp 官方 release notes<sup>[官方 12]</sup>；具体次版本号请参考最新文档。
