...
- **ObjectARX 是 AutoCAD 的 [in-process](/glossary#in-process) 原生 C++ SDK**<sup><a href="https://en.wikipedia.org/wiki/ObjectARX" target="_blank" rel="noreferrer">[百科 1]</a></sup>，扩展模块编译为 `.arx` / `.dbx`（实质是 Windows DLL，文件扩展名不同），加载到 AutoCAD 进程内，可直接访问 AutoCAD 内部 C++ 类<sup><a href="https://en.wikipedia.org/wiki/ObjectARX" target="_blank" rel="noreferrer">[百科 1]</a></sup>。在 AutoCAD API 体系的层级中，ObjectARX 处于最底层位置，能力最深，学习与维护成本也较高。
...
- **2022 年 12 月 7 日 Forge 改名为 Autodesk Platform Services（[APS](/glossary#aps-autodesk-platform-services)）**<sup>[官方 6]</sup>：这是 Autodesk 云战略品牌升级的关键节点。
...
- **Visual LISP 仍是设计师友好层**：从 AutoCAD R12 (1992) 以来一直是核心扩展通道，AutoCAD 2026 仍内置 Visual LISP IDE<sup><a href="https://www.autodesk.com/products/autocad/features" target="_blank" rel="noreferrer">[官方 8]</a></sup>。
...
AutoCAD 1.0 发布于 1982 年 12 月<sup><a href="https://en.wikipedia.org/wiki/AutoCAD" target="_blank" rel="noreferrer">[百科 4]</a></sup>，是早期 PC 上的 CAD 工具。早期版本通过 AutoLISP（基于 Lisp 的脚本）和 ADS（AutoCAD Development System，C 语言 SDK）扩展。
...
AutoCAD R13（1994 末 / 1995 初）首次引入运行时扩展架构 **ARX**<sup><a href="https://en.wikipedia.org/wiki/ObjectARX" target="_blank" rel="noreferrer">[百科 1]</a></sup>——基于 C++，替代 ADS 成为底层 SDK；该 API 在 R14（1997）时代正式更名为 **ObjectARX**。从此开始 AutoCAD 平台具备"in-process 原生 C++ 扩展"能力，奠定了 30 年来的 API 哲学基础。

### 1.3 R14、2000 系列（1997–2002）

- AutoCAD R14（1997）：进一步成熟，普及 ObjectARX 应用
- AutoCAD 2000（1999）：引入多文档支持、Transaction 模式、Visual LISP IDE 集成<sup><a href="https://en.wikipedia.org/wiki/AutoCAD" target="_blank" rel="noreferrer">[百科 4]</a></sup>

### 1.4 Managed .NET API：AutoCAD 2006

AutoCAD 2006 引入 Managed .NET API<sup><a href="https://www.cadtrainingonline.com/introduction-to-autocad-net-api-basics/" target="_blank" rel="noreferrer">[第三方 9]</a></sup>，作为 ObjectARX 的托管 wrapper。这一变化的战略意义：

- 降低 C++ 学习门槛（C# 比 C++ 更友好）
- 利用 .NET Framework 生态（WPF、Forms、网络库）
- 但 .NET API 仍只是 ObjectARX 的薄包装——大多数 .NET 类对应一个底层 C++ 类<sup><a href="https://www.cadtrainingonline.com/introduction-to-autocad-net-api-basics/" target="_blank" rel="noreferrer">[第三方 9]</a></sup>
...
- AutoCAD 2011：DWG 格式升级到 2010 版本（`AC1024`）<sup><a href="https://en.wikipedia.org/wiki/AutoCAD" target="_blank" rel="noreferrer">[百科 4]</a></sup>
- AutoCAD 2013：DWG 格式升级到 2013 版本（`AC1027`）<sup><a href="https://en.wikipedia.org/wiki/AutoCAD" target="_blank" rel="noreferrer">[百科 4]</a></sup>
...
| AutoCAD 2026 | VS2022 v17.14 | .NET 8.0 | AC1032 | 2D 文件打开声称 11x faster<sup><a href="https://www.autodesk.com/blogs/autocad/autocad-2026/" target="_blank" rel="noreferrer">[新闻 7]</a></sup>; Smart Blocks AI; Activity Insights What's Changed; Connected Support Files; Autodesk Assistant<sup><a href="https://www.autodesk.com/products/autocad/features" target="_blank" rel="noreferrer">[官方 8]</a></sup> |
...
根据 Autodesk 官方 AutoCAD 2026 features 页面与官方博客<sup><a href="https://www.autodesk.com/products/autocad/features" target="_blank" rel="noreferrer">[官方 8]</a><a href="https://www.autodesk.com/blogs/autocad/autocad-2026/" target="_blank" rel="noreferrer">[新闻 7]</a></sup>：
...
- **性能**：声称 2D 文件打开 11x faster, 启动 4x faster vs AutoCAD 2025<sup><a href="https://www.autodesk.com/blogs/autocad/autocad-2026/" target="_blank" rel="noreferrer">[新闻 7]</a></sup>

> **[评论]** "11x faster"是 Autodesk 内部基准测试结果，实际效果因系统配置和文件复杂度而异<sup><a href="https://blog.cadalyst.com/cadmanagement/autocad-2026-focuses-on-productivity-with-speed-ai-and-collaboration" target="_blank" rel="noreferrer">[新闻 13]</a></sup>。营销话术与真实体验之间通常有 gap，建议对此类数字保持谨慎乐观。
...
ObjectId 是 AutoCAD 数据库的核心抽象<sup><a href="https://help.autodesk.com/view/OARX/2026/ENU/" target="_blank" rel="noreferrer">[官方 10]</a></sup>：
...
ObjectId 内存中是 64 位指针的语义包装，持久化到 DWG 时编码为 handle（一个 16 进制字符串）<sup><a href="https://help.autodesk.com/view/OARX/2026/ENU/" target="_blank" rel="noreferrer">[官方 10]</a></sup>。
...
`acedRegCmds->addCommand` 注册一个命令到 AutoCAD 命令行<sup><a href="https://help.autodesk.com/view/OARX/2026/ENU/" target="_blank" rel="noreferrer">[官方 10]</a></sup>，用户输入 `MYCMD` 即调用 `MyCommandFunc`。
...
通过派生 `AcDbEntity` 实现自定义图元<sup><a href="https://help.autodesk.com/view/OARX/2026/ENU/" target="_blank" rel="noreferrer">[官方 10]</a></sup>：
...
**Object Enabler** 机制：当不安装 Custom Entity 的定义 DLL 时，AutoCAD 用 proxy graphics 显示<sup><a href="https://help.autodesk.com/view/OARX/2026/ENU/" target="_blank" rel="noreferrer">[官方 10]</a></sup>。这让垂直产品（AutoCAD Civil 3D、AutoCAD Plant 3D 等）的特殊对象在标准 AutoCAD 中也可被读取显示。
...
AutoLISP 自 AutoCAD 1.0（1982）起即是核心扩展通道<sup><a href="https://en.wikipedia.org/wiki/AutoCAD" target="_blank" rel="noreferrer">[百科 4]</a></sup>。Visual LISP 在 R12（1992）/ AutoCAD 2000 时代成熟为带 IDE 的 Lisp 方言。

> **[评论]** 即使在 2026 年，AutoCAD Vertical Toolsets 中仍内置 Visual LISP IDE<sup><a href="https://www.autodesk.com/products/autocad/features" target="_blank" rel="noreferrer">[官方 8]</a></sup>——这是 Autodesk 对设计师友好性的长期承诺。在 ObjectARX、.NET、COM 之后增加 Visual LISP 而不是替换，反映了"加法而非替代"的兼容哲学。
...
AutoCAD 提供三层扩展数据机制<sup><a href="https://help.autodesk.com/view/OARX/2026/ENU/" target="_blank" rel="noreferrer">[官方 10]</a></sup>，从轻到重：
...
XData 用 DXF group code 表示，附在 entity 上。**16K 大小限制**，结构松散，需自行解析<sup><a href="https://help.autodesk.com/view/OARX/2026/ENU/" target="_blank" rel="noreferrer">[官方 10]</a></sup>。
...
DWG 文件格式封闭性是 Autodesk 的核心战略资产<sup><a href="https://en.wikipedia.org/wiki/Open_Design_Alliance" target="_blank" rel="noreferrer">[百科 5]</a></sup>。即便 [ODA](/glossary#oda-open-design-alliance)（Open Design Alliance）提供了 [Drawings SDK](/glossary#oda-drawings-sdk-前-teigha) 反向工程读写 DWG，Autodesk 仍持续主导格式演进，让兼容厂商始终落后一个版本周期。

> **[评论]** 在样本平台中，**BricsCAD 是 DWG 兼容路径的代表** [3.9 §一 历史演进](/platforms/bricscad#一、历史演进)；[3.9 §七 与 AutoCAD 的兼容承诺](/platforms/bricscad#七、与-autocad-的兼容承诺-详细分析)——它不仅 DWG 文件兼容，BRX SDK 还做到与 ObjectARX **源码兼容**（同源码 + 不同编译目标 = 两套二进制）。这种"API 层兼容"是 ObjectARX 在样本中唯一被外部平台明确镜像的设计。
...
- **AutoCAD 工具链节奏数据**已根据 Autodesk 官方 ObjectARX SDK 文档<sup><a href="https://aps.autodesk.com/developer/overview/objectarx-autocad-sdk" target="_blank" rel="noreferrer">[官方 2]</a></sup>与第三方博客<sup>[第三方 3]</sup>核实更新；具体 VS 子版本号与 .NET 子版本以最新文档为准。
- **AutoCAD 2027 工具链信息**来自第三方博客<sup>[第三方 3]</sup>；官方 ObjectARX SDK 2027 文档发布后应以官方为准。
- **Forge → APS 改名时间**为 2022 年 12 月 7 日<sup>[官方 6]</sup>。
- **AutoCAD 2026 性能数据"11x faster"**来自 Autodesk 内部基准测试，实际效果因系统配置和文件复杂度而异<sup><a href="https://blog.cadalyst.com/cadmanagement/autocad-2026-focuses-on-productivity-with-speed-ai-and-collaboration" target="_blank" rel="noreferrer">[新闻 13]</a></sup>。
