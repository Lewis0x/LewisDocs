...
>
> **本文档定位**：基于 9 个样本 CAD 平台 API 深度剖析，从 **UI 框架** 这一单一维度做横向提取。是文档 2（厂商 API 横向对比）下的"UI 框架"分册。
>
...
- **桌面 CAD 在 2017–2020 间集体迁移到 <Term def="Chromium Embedded Framework：把 Chromium 渲染引擎打包成 C++ 库，让原生应用嵌入完整 HTML/CSS/JS 运行时。binary ~150MB（自带 Chromium）。是桌面应用嵌入 web UI 的事实标准。">CEF</Term>**：SketchUp 2017 引入 `UI::HtmlDialog` 替代旧 WebDialog [3.7 §三 现代 UI::HtmlDialog（SU 2017+）](/platforms/sketchup#三、ruby-api-核心对象层级)；AutoCAD / SolidWorks 也在多个对话框嵌 Chromium。背后是"用 Web 写 ISV 插件 UI 比手写 MFC 快 10 倍"的工程现实。
- **NX Block UI Styler 是样本中唯一"开发者不写 UI 代码"的设计**：拖放定义 dialog blocks，编译器自动生成 4 种语言（C++ / C# / Python / Java）的对等 stub 代码 [3.3 §二 Common API 的"四语言对等"哲学](/platforms/nx#二、api-整体架构-classic-common-双层并存)。这是"四语言对等"在 UI 层的延伸——也是样本中唯一一个把 UI 当成"声明式资源"而非代码的 CAD。
- **Onshape 是唯一"完全没有桌面 UI 框架问题"的样本**：所有 UI 都是浏览器端 React + WebGL，用 REST 调用驱动 [3.4 §三 git 式四层数据模型](/platforms/onshape#三、核心数据模型-git-式四层)；FeatureScript 驱动 feature dialogs。代价是"必须接受浏览器作为 UI 边界"——CSP、扩展冲突、版本碎片都成为 UI 风险。
- **FreeCAD 的 <Term def="FreeCAD 特色的“非弹窗对话框”模式：把对话框嵌成左侧工具栏的临时面板，参数化编辑期间 viewport 操作不被打断。Qt + Python 实现。SolidWorks PropertyManager Page (PMP) 是同类思路的商业版本。">task panel</Term>** 模式是开源 CAD 中讨论最多的设计 [3.8 §三 5 层架构](/platforms/freecad#三、python-嵌入式哲学-双向集成到极致)：把对话框嵌成左侧工具栏临时面板，无需弹窗中断 viewport 操作。**SolidWorks PMP（PropertyManager Page）** 是同类思路的商业版本 [3.6 §四 IModelDocExtension 与 PMP](/platforms/solidworks#四、selectbyid2-mark-机制)。
...
2. **CEF 在 SketchUp 中的引入时间点**：SU 2017 替代旧 WebDialog 提供 `UI::HtmlDialog` API [3.7 §三 现代 UI::HtmlDialog（SU 2017+）](/platforms/sketchup#三、ruby-api-核心对象层级)；其余样本平台 CEF 引入时间在 2017±2 年区间。
3. **NX Block UI Styler 自动生成 4 种语言代码**：C++、C#、Python、Java；不直接生成 VB.NET，但 .NET 程序集可被 VB.NET 调用 [3.3 §二](/platforms/nx#二、api-整体架构-classic-common-双层并存)。
4. **AutoCAD 的 6 层 UI 通道**：AutoLISP（最薄）→ VBA / Visual LISP IDE → COM Automation → .NET → ObjectARX C++（最深）→ Web Browser API [3.1 §二 API 整体架构：六层金字塔](/platforms/autocad#二、api-整体架构-六层金字塔)。
...
6. **iTwin.js Frontend 是纯 React 组件库**：Frontend / Backend / Common 三段架构中，Frontend 完全是浏览器端 React + Three.js / WebGL，与 MicroStation 桌面 UI 完全独立 [3.5 §五 iTwin.js 三段架构](/platforms/microstation#五、扩展数据元金字塔-linkage-xattribute-item-types-ecinstance)。
7. **SolidWorks PMP 的具体技术名**：`IPropertyManagerPage2` 接口；UI 自动嵌成左侧 Feature Manager 的子面板 [3.6 §四](/platforms/solidworks#四、selectbyid2-mark-机制)。
8. **FreeCAD UI 是 Qt 5 / Qt 6**：Qt Designer 生成 `.ui` XML 文件，运行时由 Gui 模块加载；FeaturePython 让 Python 类直接成为 DocumentObject 的 UI 渲染对象 [3.8 §四 FeaturePython](/platforms/freecad#四、workbench-体系-模块化的功能组织)。
...
⚠️ SketchUp SU 2025 引入的 "Logical Pixels" API 就是这个问题的迟到回应 [3.7 §六 SU 2025 新引入的逻辑像素](/platforms/sketchup#六、ui-htmldialog-web-集成的现代化路径)——其他 CAD 在 1990s–2000s 就解决了。
...
NX Builder + Commit [3.3 §三 Common Object Model：Session/Part/Feature/Builder](/platforms/nx#三、common-object-model-session-part-feature-builder)、CATIA Spec/Result/Update [3.2 §六 Spec/Result/Update：CATIA 特征建模哲学](/platforms/catia#六、spec-result-update-三段式-catia-特征建模哲学)、FreeCAD task panel——都是 CAD UI 对这种 lifecycle 的特殊支持。普通 UI 框架没有这个 lifecycle。
...
SU 的 `UI::HtmlDialog` + JavaScript callback 就是这种桥的样板 [3.7 §三 现代 UI::HtmlDialog（SU 2017+）](/platforms/sketchup#三、ruby-api-核心对象层级)。
...
### 2.1 原生桌面 toolkit
- **MFC**：AutoCAD ObjectARX 原生对话框 [3.1 §三](/platforms/autocad#三、对象模型-database-editor-document)、SolidWorks 早期 ActiveX
- **Qt**：FreeCAD 基础 toolkit + Qt Designer `.ui` 文件 [3.8 §三](/platforms/freecad#三、python-嵌入式哲学-双向集成到极致)、CATIA 部分内部组件
...
- **WinForms**：AutoCAD .NET ObjectARX wrapper、SolidWorks Add-in、MicroStation DgnPlatformNET [3.5 §四](/platforms/microstation#四、几何核心-dgngeometry-curvevector-smartsolid)
- **WPF**：AutoCAD 2010+ palette、MicroStation CONNECT 现代 UI
- **专有 .NET 控件**：SolidWorks <Term def="PropertyManager Page：SolidWorks 特有的“侧栏对话框”控件，开发者实现 IPropertyManagerPage2 接口；UI 自动嵌成左侧 Feature Manager 子面板。是参数化建模 commit/rollback 范式在 .NET 的实现。">PMP</Term>（PropertyManager Page）[3.6 §四 IModelDocExtension 与 PMP](/platforms/solidworks#四、selectbyid2-mark-机制)

特点：开发速度快于原生 C++，生态丰富（NuGet 控件库），但对 Win32 出身的老 CAD 需要 .NET wrapper 桥。

### 2.3 嵌入 Web（CEF / WebView2）

- **CEF**：SketchUp `UI::HtmlDialog` (SU 2017+) [3.7 §三](/platforms/sketchup#三、ruby-api-核心对象层级)、AutoCAD Web Browser API、SolidWorks 部分对话框
...
- **NX Block UI Styler**：拖放定义 blocks，编译器生成 C++ / C# / Python / Java stub [3.3 §二](/platforms/nx#二、api-整体架构-classic-common-双层并存)——样本中**唯一**"开发者不写 UI 代码"的设计
- **FreeCAD task panel**：Qt + Python 写的左侧贴边面板，参数化建模 commit/rollback 按钮原生集成 [3.8 §三](/platforms/freecad#三、python-嵌入式哲学-双向集成到极致)
- **CATIA Modeler dialog**：CAA Object Modeler 提供的对话框组件，以 NLS 渲染 [3.2 §三](/platforms/catia#三、workspace-framework-module-源码物理组织)

特点：减少样板代码、强制一致的 UI 风格，但 customization 自由度受限；适合"一致性 > 灵活性"的取舍。

### 2.5 纯 Web

- **Onshape**：React 前端 + WebGL viewport [3.4 §三](/platforms/onshape#三、核心数据模型-git-式四层)，所有操作通过 REST 调用，FeatureScript 驱动 feature dialogs
- **iTwin Viewer**：Bentley 的 React 嵌入式组件 [3.5 §五](/platforms/microstation#五、扩展数据元金字塔-linkage-xattribute-item-types-ecinstance)，把 iModel 的 3D 视图 + 属性面板封成可嵌的 npm 包
...
| **BricsCAD** | **Qt / QML** ⭐（2021–2022 迁移中）+ MFC 兼容层（`BcUiPanelMFC`） | BrxMgd.dll（与 AutoCAD .NET 源码兼容） | 未观察到 | 无（QML 声明式但非 dialog 框架） | BRX C++ + .NET |
...
样本中可观测的时间点：
- **SketchUp 2017**（2016-Q4）：`UI::HtmlDialog` 替代旧 WebDialog [3.7 §三](/platforms/sketchup#三、ruby-api-核心对象层级)
...
### 5.1.1 BricsCAD 走"第三条路"：Qt/QML 而非 CEF

样本中 **BricsCAD 在 2021–2022 选择了不同方向**——不上 CEF 嵌 Web，而是把整个 UI 从 wxWidgets/MFC 迁移到 **Qt/QML** [3.9 §六 UI 架构](/platforms/bricscad#六、ui-架构-从-wxwidgets-mfc-到-qt-qml)。Bricsys 在 Qt 官方案例研究中给出的选择动机：

| 动机 | BricsCAD 的考虑 |
|---|---|
| 跨平台一致性 | 一份代码同时支持 Windows / macOS / Linux |
| 像素级设计还原 | wxWidgets/MFC 时代设计师 mockup 难以一致 |
| WebAssembly 演进空间 | Qt 6 编译到 WASM 给"将来上 Web"留选项 |
| QML 声明式开发提速 | 比传统 C++ widget 节奏快 |
| 进程模型 | Qt 同进程（vs CEF 多进程 Chromium 的内存开销）|

⭐ **与 CEF 路线对比**：

| 维度 | 路线 A 原生 widget | 路线 B 嵌 CEF | BricsCAD：Qt/QML |
|---|---|---|---|
| UI 描述方式 | 命令式（C++ widget） | 声明式（HTML/CSS） | 声明式（QML） |
| 跨平台 | Windows-centric | 完全跨平台 | 完全跨平台 |
| 进程模型 | 同进程 | 多进程（Chromium 重） | 同进程 |
| 内存占用 | 低 | 高 | 中等 |

关键工程治理细节 [3.9 §六 UI 架构](/platforms/bricscad#六、ui-架构-从-wxwidgets-mfc-到-qt-qml)：
- 完整迁移**初步估计 50 人年**（跨多个版本逐步推进）
- **MFC 兼容层保留**——`BcUiPanelMFC` 让 ISV 可在面板嵌 Qt 控件，不强制立即重写
- V24 RIBBON 命令是早期切换到 Qt/QML 的代表性模块

> **[评论]** 这条路的成本是 Bricsys 内部需要长期维护两套 UI 体系，但保留了 ISV 投资。FreeCAD 早就用 Qt（widget 风格），NX 长期用 Qt（widget 风格），但 **BricsCAD 是样本中唯一选择 Qt/QML 声明式 UI** 的——QML 让 UI 描述代码量显著减少，跨平台一致性最强；代价是 QML 对复杂 CAD 交互（实时 3D viewport、复杂手势）仍需配合 C++ 自定义 item。
...
这种"declare once, generate everywhere"模式 [3.3 §二](/platforms/nx#二、api-整体架构-classic-common-双层并存) 的价值：
...
这是 Onshape vs 桌面 CAD 的根本张力 [3.4 §一](/platforms/onshape#一、历史背景与所有权变迁)。Web-first 是 SaaS 商业模式的必要选择，但牺牲了离线 + 性能极致。
...
本文档为本系列 9 个样本平台 API 深度剖析的横向"UI 框架"提取。主要事实回链已在正文中标注 `[回链：3.x §y]`，由 `rewrite_links.py` 自动展开为站内可点击链接。
...
### [第三方]
- [第三方 1] 本系列 9 篇厂商深度剖析（[3.1 AutoCAD](/platforms/autocad) · [3.2 CATIA](/platforms/catia) · [3.3 NX](/platforms/nx) · [3.4 Onshape](/platforms/onshape) · [3.5 MicroStation](/platforms/microstation) · [3.6 SolidWorks](/platforms/solidworks) · [3.7 SketchUp](/platforms/sketchup) · [3.8 FreeCAD](/platforms/freecad) · [3.9 BricsCAD](/platforms/bricscad)）— 本文 UI 章节具体事实的一手来源
...
本文档大部分论断为 B 级（基于本系列 9 个样本平台的归纳）；带 `[证据等级 C]` 标记的论断需结合实际 CAD 团队访谈或市场调研进一步验证。
