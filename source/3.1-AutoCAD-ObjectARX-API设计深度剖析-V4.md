...
DWG 文件格式封闭性是 Autodesk 的核心战略资产<sup>[百科 5]</sup>。即便 [ODA](/glossary#oda-open-design-alliance)（Open Design Alliance）提供了 [Drawings SDK](/glossary#oda-drawings-sdk-前-teigha) 反向工程读写 DWG，Autodesk 仍持续主导格式演进，让兼容厂商始终落后一个版本周期。

> **[评论]** 在样本平台中，**BricsCAD 是 DWG 兼容路径的代表** [回链：3.9 §一 历史演进；3.9 §七 与 AutoCAD 的兼容承诺]——它不仅 DWG 文件兼容，BRX SDK 还做到与 ObjectARX **源码兼容**（同源码 + 不同编译目标 = 两套二进制）。这种"API 层兼容"是 ObjectARX 在样本中唯一被外部平台明确镜像的设计。
