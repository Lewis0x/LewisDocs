...
> 本文档是首次系统性核实 8 篇厂商深度文档（3.1 - 3.8）所有引用的产物。3.9 BricsCAD 系后续新增（2026-05-10），本表暂未覆盖；下次重跑 `audit_citations.py` 时可一并纳入。
...
| 维度 | 数 |
|---|---:|
| 扫描文档 | 8 篇厂商深度（3.1-3.8）；3.9 BricsCAD 暂未纳入 |
| 引用 marker 总数 | 704 |
| 唯一 URL 数 | 88 |
...
**这次能给的保证：**
- 8 篇厂商文档的全部 704 条引用 marker 已被机械索引（3.9 BricsCAD 待纳入）
