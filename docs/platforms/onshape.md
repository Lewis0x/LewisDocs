...
4. **Topology ID 跨 microversion 转换**：Topology ID 在某 microversion 中定义后，可被 Onshape 翻译为当前 microversion 中的对应 ID 集合<sup><a href="https://onshape-public.github.io/docs/api-intro/architecture/" target="_blank" rel="noreferrer">[官方 1]</a></sup>。这解决了"参数化建模中拓扑命名问题"——Onshape 团队称为 [Associativity](/glossary#关联性-associativity)（[关联性](/glossary#关联性-associativity)）机制，由 Topology ID 跨 microversion 翻译实现。
...
- **Part Studio**：feature 列表、<Term def="把 NURBS / B-Rep 曲面近似为三角面片的过程，用于显示与导出（STL、glTF 等）。曲面精度越高，生成的三角面片越多。">tessellation</Term>、几何查询
- **Assembly**：装配关系、<Term def="装配体中限定零件相对位置的约束（同心 / 共面 / 距离 = 10mm 等）。Onshape / SolidWorks 等用此术语；CATIA 称 constraint，FreeCAD 称 placement。">mate</Term> 操作
- **Drawing**：工程图、视图操作
