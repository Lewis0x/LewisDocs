...
4. **Topology ID 跨 microversion 转换**：Topology ID 在某 microversion 中定义后，可被 Onshape 翻译为当前 microversion 中的对应 ID 集合<sup>[官方 1]</sup>。这解决了"参数化建模中拓扑命名问题"——Onshape 团队称为 [Associativity](/glossary#关联性-associativity)（[关联性](/glossary#关联性-associativity)）机制，由 Topology ID 跨 microversion 翻译实现。
