...
3. **几何复杂度高**：参数化建模、特征关联性、拓扑命名等问题难以彻底解决——商业 CAD 经过 40 年仍在持续改进，开源 CAD 直到 2024 年 FreeCAD 1.0 才有较成熟的 TNP 修复方案 [回链：3.8 §五 TopoShape 与 ElementMap]。
...
> **[判断]** 机制 D（Synchronous Technology）在用户体验上具有明显优势 [回链：3.3 §四 Synchronous Technology]，但工程难度较高。务实路径：从机制 B（DAG 依赖图）起步，在成熟后增加 Synchronous Technology 等价能力。此判断针对追求"导入第三方文件直接编辑 + 关联性维持"场景；不强调多 CAD 互操作的封闭工作流，机制 B 已足够。
...
⭐ **观察**：在工程数据集成场景中，Level 5（元元模型）显示明显价值 [回链：3.5 §六 ECObjects / ECSchema；3.5 §十 独特设计哲学]——MicroStation/iTwin 在基础设施工程的核心竞争力之一来自 ECSchema/BIS。
...
⭐ **观察**：在本系列覆盖的 8 个样本中，Onshape 是较少见的实现完整 Git 式 CAD 协作的平台 [回链：3.4 §三 git 式四层数据模型]。
...
AutoCAD 的 LISP 1982 年至今可用 [回链：3.1 §五 AutoLISP / Visual LISP]——长期承诺是平台权威的来源之一。
