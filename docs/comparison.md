...
⭐ **观察**：在本系列覆盖的 9 个样本中，Onshape 是较少见的实现完整 Git 式 CAD 协作的平台 [3.4 §三 git 式四层数据模型](/platforms/onshape#三、核心数据模型-git-式四层)。Bentley iTwin 的 ChangeSet 接近但仍是线性。商业 CAD 主流仍是 PDM check-out/check-in。
