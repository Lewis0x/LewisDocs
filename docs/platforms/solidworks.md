...
- **SolidWorks 在中端机械 CAD 中有较广泛使用**：1995 年由 MIT 出身的 Jon Hirschtick 创立，1997 年被 Dassault Systèmes 以 3.1 亿美元收购<sup>[百科 1]</sup>，至今 2025 年正好 30 周年<sup>[新闻 2]</sup>。它把"航空航天级 CATIA 的能力下放到中小企业能负担"作为核心战略。
- **SolidWorks API 是 COM-based 出身、向多语言开放**：核心是 `SldWorks.Application` COM 对象，外加扩展 IDispatch 双重接口。VBA 是最普及的入门路径，但 C#/VB.NET/C++ 通过 Type Library 都可访问。⚠️ 与 ObjectARX 不同的是，SolidWorks API 没有 in-process 原生 C++ SDK 这一层——所有扩展都通过 COM/Add-in 层走<sup>[官方 3]</sup>。
- **核心几何内核是 Parasolid**：Siemens PLM 提供的工业级 B-Rep 内核<sup>[百科 4]</sup>。⚠️ 这是一个有趣的产业事实——SolidWorks（DS 旗下）和 NX/Solid Edge（Siemens 旗下）使用同一个底层内核。这种"竞争对手共享内核"在样本平台中较罕见。
- **每年 5 个 Functional Delivery (FD)**：⚠️ **2025+ 重大节奏变化**——SolidWorks 2026 起明确 "**每 10 周一个 FD release**"<sup>[官方 5][新闻 6]</sup>，年度版本号只是 5 个 FD 中的"第一个 + 第五个"标志，全年持续交付新功能。
- **2026 引入 Aura AI 虚拟伴侣**<sup>[官方 7]</sup>：3DEXPERIENCE 平台的 AI 助手集成到 SolidWorks Design 中；同时 AI 自动生成 Drawing 功能上线（从零件/装配自动产出工程图）<sup>[新闻 8]</sup>。
- **IModelDocExtension 扩展接口模式**：SolidWorks 在原始接口（`IModelDoc2`）上叠加 `IModelDocExtension`，提供新方法而不破坏老接口<sup>[官方 3]</sup>。这是"接口版本化"的优雅范式。
- **SelectByID2 的 mark 参数**：选择对象时可指定 mark 整数，让 feature 在多个被选对象间区分角色（如 base/path）<sup>[官方 3][第三方 9]</sup>。这是 SolidWorks API 独有的"角色化选择"设计。
- **Document Manager API 是离线读 SLDPRT/SLDASM/SLDDRW 的独立 API**<sup>[官方 10]</sup>：不需打开 SolidWorks 即可读取文件元数据。
- **PDM API 是文档管理的独立 API**<sup>[官方 10]</sup>：SolidWorks PDM Standard/Professional 各自的 API，独立于 SolidWorks 主 API。
- **Hirschtick 创始人故事**：创立 SolidWorks 后回归再创业 Onshape（2012）和 Frustum（2018 被 PTC 收购）<sup>[百科 1]</sup>——他在 CAD 行业的 30 年弧线本身就是一部"产品哲学演进史"。

---

## Key Findings

1. **Hirschtick 创立 SolidWorks**：MIT 校友、原 Computervision 工程师、Premise（前 SolidWorks 雏形）创始人 Jon Hirschtick 用赌博团队（21 点）赢得的资金创立 SolidWorks Corp.<sup>[百科 1]</sup>。1993 年成立，1995 年 11 月发布 SolidWorks 95<sup>[百科 1]</sup>。
2. **DS 收购**：1997 年 6 月 Dassault Systèmes 以 3.1 亿美元收购 SolidWorks Corp.<sup>[百科 1]</sup>。
3. **SolidWorks 内核**：Parasolid（Siemens 提供）<sup>[百科 4]</sup>，**不是** DS 自家的 CGM。这是 DS 在并购时保留的关键技术决策——切换到 CGM 风险过大。
4. **核心入口对象**：`SldWorks.Application`（COM 自动化对象）<sup>[官方 3]</sup>。从 `SldWorks` 对象获取 `IModelDoc2`、`IPartDoc`、`IAssemblyDoc`、`IDrawingDoc` 等。
5. **API Type Library 文件**：`sldworks.tlb`（位于 SolidWorks 安装目录）。可在 VBA、C#、VB.NET、C++ 中导入引用。
6. **IModelDocExtension 模式**<sup>[官方 3]</sup>：每个 Document 类型有 `Extension` 属性，返回 `IModelDocExtension` 对象，提供"新版"方法。这是 API 演进时不破坏老代码的优雅做法。
7. **SelectByID2 的 7 个参数**<sup>[官方 3][第三方 9]</sup>：name, type, X, Y, Z, append, mark——其中 mark 是整数，用于在 feature 创建时区分被选对象的角色。
8. **2026 API 新接口**<sup>[新闻 11]</sup>：`IDSPBRMaterial`（DSPBR 材质）、`IFamilyTableAnnotation`（族表注释）、`IFamilyTableFeature`（族表特征）。
9. **SolidWorks 2026 是 30 周年版**<sup>[新闻 2][官方 7]</sup>：1995–2025/2026 整 30 年。Aura AI 与自动生成 Drawing 是其 30 周年标志性功能。
10. **每年 5 FD**<sup>[官方 5][新闻 6]</sup>：⚠️ 2025+ 起明确"每 10 周一个 Functional Delivery"，年度版本号不再是主要的发布节奏。
11. **3 大 API 套件**<sup>[官方 10][新闻 11]</sup>：SolidWorks API（CAD 主体）、Document Manager API（离线文件读取）、SolidWorks PDM API（文档管理）。三者独立。
12. **Onshape 由 Hirschtick 回归创立**：2012 年 Hirschtick 与多位 SolidWorks 老兵共同创立 Onshape<sup>[百科 1]</sup>，这是同一位 CAD 产品哲学家的"重做"——文档 3.4 已专门讨论。
...
Jon Hirschtick 是 MIT 出身、Computervision 工程师，1980s 末加入 Premise（一家早期 PC CAD 创业公司）<sup>[百科 1]</sup>。
...
1995 年 11 月 SolidWorks 95（v1.0）发布<sup>[百科 1]</sup>，这是首批基于 Windows 95 / NT 的中端 3D CAD 之一。核心定位：
...
1997 年 6 月 Dassault Systèmes 以 3.1 亿美元收购 SolidWorks Corp.<sup>[百科 1]</sup>。但 DS 选择**保持 SolidWorks 品牌独立运营**——SolidWorks 仍在马萨诸塞州 Concord 总部，独立的 R&D 团队，独立的产品路线。
...
- 2009 年：注册用户突破 100 万
- 2010s 早期：在中端 CAD 市场有较广泛的使用——"CAD/CAM 教育普遍以 SolidWorks 为入门工具"<sup>[百科 1]</sup>

### 1.5 Hirschtick 离开与 Onshape 创立（2011–2012）

2011 年 Hirschtick 从 SolidWorks 离开，2012 年与多位 SolidWorks 老兵（包括前 SolidWorks CTO 等）共同创立 **Onshape**<sup>[百科 1]</sup>。
...
⚠️ **重大节奏变化**<sup>[官方 5][新闻 6]</sup>：从 2024+ 起 DS 明确"**SolidWorks 每 10 周一个 Functional Delivery (FD) release**"，全年 5 个 FD：
- FD0（年度大版本）：每年秋季
...
| SolidWorks 2026 | **30 周年**；**Aura AI** 虚拟伴侣<sup>[官方 7]</sup>；AI 自动生成 Drawing<sup>[新闻 8]</sup>；DSPBR（Dassault Systèmes Physically Based Rendering）材质<sup>[新闻 13]</sup>；**API 新增 IDSPBRMaterial / IFamilyTableAnnotation / IFamilyTableFeature**<sup>[新闻 11]</sup>；背景导入大型非原生文件<sup>[新闻 13]</sup> |
...
⚠️ **重要事实**：SolidWorks **没有 in-process 原生 C++ SDK 的单独入口**<sup>[官方 3]</sup>。所有扩展都通过 COM Automation 层走——这与 ObjectARX 的"C++ 原生为主、.NET 是 wrapper"路线不同。
...
SolidWorks 在 API 演进中保持向后兼容的关键设计：当 `IModelDoc2` 需要增加新方法时，**不直接修改 `IModelDoc2`**（会破坏 COM 兼容），而是在 `IModelDocExtension` 中增加新方法<sup>[官方 3]</sup>。
...
mark 参数是关键设计。它是一个整数，让 feature 在创建时区分被选对象的角色<sup>[官方 3][第三方 9]</sup>：
...
| 平台 | 选择对象的角色区分 |
|---|---|
| **SolidWorks** | mark 整数，feature API 通过 mark 识别角色<sup>[官方 3]</sup> |
...
SolidWorks Add-in 是 in-process DLL，需实现 `ISwAddin` 接口<sup>[官方 3]</sup>：
...
Add-in 通过 Windows 注册表注册到 SolidWorks<sup>[官方 3]</sup>：
...
SolidWorks 事件通过 COM connection points 实现<sup>[官方 3]</sup>。在 .NET 中通过 events 暴露：
...
SolidWorks 事件按文档类型分类<sup>[官方 3]</sup>：
- `DocumentEvents`（通用）
...
SolidWorks 文件支持"自定义属性"（Custom Properties）<sup>[官方 3]</sup>，分两类：
...
SolidWorks Equation Manager 提供基于全局变量与方程的参数化建模<sup>[官方 3]</sup>：
...
SolidWorks 2026 引入 **Aura AI**——3DEXPERIENCE 平台的 AI 虚拟伴侣集成到 SolidWorks Design 中<sup>[官方 7]</sup>：
- 应用内 AI 助手
- 自然语言查询设计意图
- 操作建议与命令搜索
...
2026 的标志性 AI 功能：从零件/装配自动生成 Drawing<sup>[新闻 8]</sup>：

> "SOLIDWORKS 2026 can now automatically generate Drawings for your Parts and Assemblies with the use of AI. Choose the Parts and Assemblies to detail, and SOLIDWORKS will intelligently determine what Model Views and Detailing are needed to convey the design"<sup>[新闻 8]</sup>
...
根据 CADSharp 2026 API 总结<sup>[新闻 11]</sup>：
...
> **[评论]** CADSharp 评价 SolidWorks 2026 API 增量为"Not the most exciting year of enhancements, but that's not a bad thing. It just goes to show how mature these APIs are."<sup>[新闻 11]</sup>——这反映了 SolidWorks API 已进入"成熟期 + 微调"阶段。新功能更多在 UX 与 AI 层，而不是 API 革命。

### 9.4 5 FD/年节奏

⚠️ **2024+ 起 SolidWorks 转向"5 FD/年"持续交付节奏**<sup>[官方 5][新闻 6]</sup>：

> "SOLIDWORKS delivers new features and performance upgrades every ten weeks throughout the year. ... SOLIDWORKS releases are thoroughly tested and maintained with regular service packs."<sup>[官方 5]</sup>

> "the annual SOLIDWORKS release is just one of five major functional delivery (FD) releases issued each year"<sup>[官方 5]</sup>
...
- [官方 5] SolidWorks 官方 What's New 页面, https://www.solidworks.com/product/whats-new
- [官方 7] DS SOLIDWORKS, "What's New in SOLIDWORKS 2026"（官方 PDF）, https://files.solidworks.com/Supportfiles/Whats_new/2026/English/whatsnew.pdf
...
- [新闻 13] TriMech, "Top 10 Features in SOLIDWORKS 2026", 2026-01, https://trimech.com/top-10-features-in-solidworks-2026/
